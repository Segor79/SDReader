...
#define PWM_BUF_LEN (6 * 8 * 2)    ///< Pack len * 8 bit * 2 LEDs	 

#define PWM_BUF_CNT 8			///< Pack 8 LEDs
...
#else
//            PWM_BUF[i + 24] = (((RGB_BUF[cnt] << i) & 0x80) > 0) ? PWM_HI : PWM_LO;
//            PWM_BUF[i + 32] = (((RGB_BUF[cnt+1] << i) & 0x80) > 0) ? PWM_HI : PWM_LO;
//            PWM_BUF[i + 40] = (((RGB_BUF[cnt+2] << i) & 0x80) > 0) ? PWM_HI : PWM_LO;
